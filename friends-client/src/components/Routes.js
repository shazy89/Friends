...
import {loadCurrentUser, loadAllUsers, userEditFunction} from '../actions/auth'
...
logOutUser = (e) => {
  ...
  return (
      <div >
  { status ? <NavBar logOutUser={this.logOutUser} setLoggedInUser={this.setLoggedInUser} history={this.history}/> : null }
      <Switch>
        <Route  exact path="/" render = {props => (<Home {...props} status={status} logOutUser={this.logOutUser} />)} />
          <Route path='/login' 
              render={props => (
                <Login {...props} />
                )} />
    ...
    export default connect(mapStateToProps, {loadCurrentUser, loadAllUsers, userEditFunction})(Routes);
